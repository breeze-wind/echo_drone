...
namespace drone {
    ...
    class DroneSerial : public sp::serialPro<head, tail> {
    public:
        enum error {
        ...
        DroneSerial() = default;
         DroneSerial(const std::string &port, int baud) : sp::serialPro<head, tail>(port, baud) {
        ...
        DroneSerial(const DroneSerial &other) = delete;

        DroneSerial(DroneSerial &&other) noexcept: sp::serialPro<head, tail>(std::move(other)) {}
